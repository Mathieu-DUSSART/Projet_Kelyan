        ...
        <select name="selectVillePoint">
            ...
            foreach($villeTab as $ville){
                ...
                if ($ville->getVilNum()==$_POST["selectVillePoint"]){
                    ...
                    if(($managerPointsDeCollecte->getNbLieuVisibleParVille($ville->getVilNum())!=0)|| isset($_SESSION["login"])){
...
if(isset($_POST["selectVillePoint"])){
    ...
    if(empty($AllPoint)){
        echo "<p class=\"lieuxPoint\">Aucun point de collecte dans cette ville</p>";
          ...
          $managerVille->delete($_POST["selectVillePoint"]);
          header('Location: index.php?page=5');
          exit;
    ...
    foreach($AllPoint as $point){
        if(($point->getPointVisibilite()==1 && !isset($_SESSION["login"]))|| isset($_SESSION["login"])){
            ...
            if($movies->status != "ZERO_RESULTS"){
                if (($movies->result[0]->address_component[2]->long_name == $managerVille->getVilNomByNum($_POST["selectVillePoint"])->getVilNom() || $movies->result[0]->address_component[3]->long_name == $managerVille->getVilNomByNum($_POST["selectVillePoint"])->getVilNom()  || $movies->result[0]->address_component[1]->long_name == $managerVille->getVilNomByNum($_POST["selectVillePoint"])->getVilNom() )){
                    $points[$i][0]=$movies->result[0]->address_component[2]->long_name;
                    $points[$i][1]=$movies->result[0]->address_component[1]->long_name;
                ...
                if(isset($_SESSION["login"])){
                    echo(" <td> <p class=\"lieuxPoint\"> Erreur : L'adresse : ". $point->getPointLieu().", ".$managerVille->getVilNomByNum($_POST["selectVillePoint"])->getVilNom()." est incorrecte , ou ne correspond pas à une adresse de la ville rentrée </p> </td>");

                    $managerPointsDeCollecte->deletePoint( $point->getPointNum());
                    $nbLieuVille=$managerPointsDeCollecte->getNbLieuParVille($_POST["selectVillePoint"]);
                    if($nbLieuVille==0){
                     echo "<p class=\"lieuxPoint\"> il n'y a pas de point de collecte a ".$managerVille->getVilNomByNum($_POST["selectVillePoint"])->getVilNom()."</p>";
                      $managerVille->delete($_POST["selectVillePoint"]);
    ...
    echo "<div id=\"map\" style =\"height:80vh; width: 44vw;margin:5% 0 0 28%\"></div>";
    if($i==0){
...
<script>
  function initMap(<?php $ville ?>) {
    var point= {lat:  <?php echo $ville[0][0] ?>, lng: <?php echo $ville[0][1] ?>};
     ...
     for ( $j = 0; $j <  count($ville)   ; $j++) {
        ...
        var pointdecollecte= {lat:<?php echo $ville[$j][0][0]?>, lng: <?php echo $ville[$j][1][0]?> };
            var <?php echo "Marker".$j ?> = new google.maps.Marker({
                ...
                icon:"image/bottle-2028855_960_720.png"

            });
            (function(<?php $j?>) {
                google.maps.event.addListener(<?php echo "Marker".$j ?>,"mouseover", function() {
                  infoWindow.close();
                  infoWindow.setContent(<?php echo " \" ".$points[$j][1].",".$points[$j][0]." \" " ?>);
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCS7YM6Xo11hhqBf94DFmJ8rZUGNV7PrXM&callback=initMap" async defer></script>
